...
per_dose as(
select last_day_of_the_week, sum(cumul) as first_sum
from date_transform
group by last_day_of_the_week
) 

select last_day_of_the_week, first_sum, CAST(first_sum AS DECIMAL) / (select * from BE_population) as first_percent
from per_dose